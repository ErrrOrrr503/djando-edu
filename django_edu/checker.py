...
class TemplateGlobalStatus(AbstractTemplate):
    ...
    def to_html(self) -> str:
        ...
        if self.tests_amount >= 0:
            res += _('<p>{passed} tests out of {total} passed.</p>').format(passed=self.passed_amount,
...
class TemplateTestStatus(AbstractTemplate):
    ...
    def to_html(self) -> str:
        if self.succeeded == True:
            return _('<div class="container d-flex"><div>Test {n}: </div><div class="text-ok">OK</div></div>').format(n=self.test_num)
        return _('<div class="container d-flex"><div>Test {n}: </div><div class="text-fail">FAIL</div></div>').format(n=self.test_num)
...
class TemplateTestReport(AbstractTemplate):
    ...
    def to_html(self) -> str:
        return _('<table class="table table-sm table-bordered-black">'
                     ...
                     '<tr class="table-bordered-black">'
                         '<td class="table-bordered-black"><span class="test-io-full multiline-text">{received}</span></td>'
                         '<td class="table-bordered-black"><span class="test-io-full multiline-text">{expected}</span></td>'
                     '</tr>'
                     '</tbody>'
                 '</table>').format(received=self.received_output,
        ...
        if task.ans_type == Task.AnsType.text:
            ...
            for test_num, test_id in enumerate(task.tests):
                ...
                if test_passed == False:
                    passed = False
                    num_failed += 1
                    # TODO: open and closed tests
                    self.report.append(TemplateTestReport(test, received))

            self.report.insert(0, TemplateGlobalStatus(passed, len(task.tests), len(task.tests) - num_failed))
        return passed
